...
// Stretch answer
const carByID = function (id) {
  let car = inventory.find(car => car.id === id);
...
let lastCar = 0;
console.log();
for(let i = lastCar; i < inventory.length; i++) {
...
let carModels = [];
for (let i = 0; i < inventory.length; i++) {
  carModels[carModels.length] = inventory[i].car_model;
}
let current = "";
for (let i = 0; i < carModels.length - 1; i++) {
  current = carModels[i];
  if (current > carModels[i+1]) {
    carModels[i] = carModels[i+1];
    carModels[i+1] = current;
    i = -1;
  }
}
console.log(carModels);
// Stretch answer
console.log(inventory.map(car => car.car_model).sort());
...
let carYears = [];
for (let i = 0; i < inventory.length; i++) {
  carYears[carYears.length] = inventory[i].car_year;
}
console.log(carYears);
// Stretch answer
console.log(inventory.map(car => car.car_year));
...
for (let i = 0; i < inventory.length; i++) {
  if (inventory[i].car_year < 2000) {
    oldCars[oldCars.length] = inventory[i];
  }
}
console.log(oldCars.length);
// Stretch answer
console.log(inventory.filter(car => car.car_year < 2000).length);
...
for (let i = 0; i < inventory.length; i++) {
  if (inventory[i].car_make === "BMW" || inventory[i].car_make === "Audi") {
    BMWAndAudi[BMWAndAudi.length] = inventory[i];
  }
}
